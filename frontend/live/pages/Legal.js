...
import { generateLegal } from "../api.js";
const tabs = [
  { id: "terms", label: "T&C" },
...
export default function Legal() {
  ...
  return (
    React.createElement('main', { className: "min-h-screen bg-slate-950 text-slate-950"  ,}
      ...
      , React.createElement('div', { className: "absolute inset-x-0 top-0 h-80 bg-gradient-to-b from-amber-300/20 via-orange-200/10 to-transparent"       ,} )

      , React.createElement('div', { className: "relative mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 py-8 sm:px-6 lg:px-8"          ,}
        , React.createElement('header', { className: "mb-8 flex flex-col gap-4 rounded-3xl border border-white/15 bg-slate-900/80 p-6 text-white shadow-2xl shadow-amber-950/20 backdrop-blur"            ,}
          , React.createElement('div', { className: "flex flex-wrap items-center justify-between gap-3"    ,}
            , React.createElement('div', null
              , React.createElement('p', { className: "text-sm font-medium uppercase tracking-[0.2em] text-amber-200"    ,}, "Legal Intelligence"

              )
              , React.createElement('h1', { className: "mt-2 text-3xl font-bold tracking-tight sm:text-4xl"    ,}, "Legal Module" )
              , React.createElement('p', { className: "mt-3 max-w-3xl text-sm text-slate-200"   ,}, "Generate legal drafts, founder agreement outline, country-specific compliance checklist, and business structure recommendation."
...
, "Back to Dashboard"
        ...
        , React.createElement('section', { className: "space-y-6 rounded-3xl border border-amber-200/60 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur md:p-8"         ,}
        ...
        , result ? (
          React.createElement('section', { className: "mt-8 space-y-6" ,}
            , React.createElement('article', { className: "rounded-2xl border border-amber-200 bg-white p-6 shadow-sm"     ,}
...
, "Export to PDF"
              ...
              , React.createElement('div', { className: "mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4"     ,}
                , React.createElement('h3', { className: "text-lg font-bold text-slate-900"  ,}, tabTitle)
                , React.createElement('pre', { className: "mt-3 whitespace-pre-wrap break-words text-sm leading-6 text-slate-700"     ,}
                  , tabContent
                )
              )
            )
            ...
            , React.createElement('div', { className: "grid gap-4 lg:grid-cols-2"  ,}
              , React.createElement('article', { className: "rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"     ,}
                , React.createElement('p', { className: "text-xs font-semibold uppercase tracking-[0.2em] text-amber-700"    ,}, "Compliance Checklist"
              ...
              , React.createElement('article', { className: "rounded-2xl border border-amber-300 bg-amber-50 p-6 shadow-sm"     ,}
                , React.createElement('p', { className: "text-xs font-semibold uppercase tracking-[0.2em] text-amber-700"    ,}, "Recommended Business Structure"

                )
                , React.createElement('h3', { className: "mt-3 text-2xl font-bold text-slate-900"   ,}
                  , result.recommended_business_structure
                )
                , React.createElement('p', { className: "mt-2 text-sm text-slate-700"  ,}, "Suggested for "
                    , result.startup_name, " operating in "   , result.country, "."
                )
              )
            )
          )
