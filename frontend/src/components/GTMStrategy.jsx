...
import { generateGtmStrategy } from "../api";
...
function SegmentCard({ index, segment }) {
  return (
    <article className="flex h-full flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div>
...
function ChannelCard({ index, channel }) {
  return (
    <article className="flex h-full flex-col gap-4 rounded-2xl border border-teal-200/80 bg-white p-5 shadow-sm">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-teal-600">
          Channel {index + 1}
        </p>
      ...
      <p className="text-sm leading-6 text-slate-600">{channel.why}</p>
      <details className="group rounded-xl border border-teal-100 bg-teal-50/60 p-4 open:bg-teal-50">
        <summary className="flex cursor-pointer list-none items-center justify-between text-sm font-semibold text-teal-900">
          <span>Action steps</span>
          <span className="text-xs font-medium uppercase tracking-wider text-teal-700 transition group-open:rotate-180">
            ▾
          </span>
        </summary>
        <ol className="mt-3 space-y-2 text-sm text-teal-900">
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <h4 className="text-base font-semibold">{phase.label}</h4>
        <span
      ...
      <ol className="space-y-2 text-sm">
        {actions.map((action, index) => (
          ...
          >
            <span className={`font-semibold ${phase.accent}`}>
              {index + 1}.
            </span>
            <span>{action}</span>
          </li>
...
function KpiCard({ kpi }) {
  return (
    <article className="flex h-full flex-col justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
...
export default function GTMStrategy() {
  ...
  return (
    <section className="mx-auto w-full max-w-6xl space-y-6 rounded-3xl border border-teal-200/70 bg-white/85 p-6 shadow-2xl shadow-teal-950/10 backdrop-blur md:p-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-sm text-slate-600">
            Translate a product, audience, and industry into segments,
            positioning, channels, pricing, a 30-60-90 plan, and the KPIs that
            prove it is working.
          </p>
        </div>
        <div className="rounded-2xl border border-teal-200 bg-teal-50 px-4 py-3 text-sm text-teal-900">
          LangChain + Gemini via OpenRouter
        </div>
      </div>
      ...
      {result ? (
        <article className="space-y-8 rounded-3xl border border-slate-200 bg-slate-50 p-6 shadow-inner">
          <header className="rounded-3xl bg-slate-900 p-6 text-white">
            ...
            <p className="mt-3 max-w-4xl text-sm leading-6 text-slate-200">
              Tailored for {audience} in the {industry} space.
            </p>
          </header>

          <section className="space-y-3 rounded-3xl border border-teal-200 bg-gradient-to-br from-teal-50 via-white to-emerald-50 p-6 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-teal-700">
            ...
            <div className="flex items-end justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Three priority segments to win in the first 90 days
                </p>
              </div>
          ...
          <section className="space-y-4">
            <div className="flex items-end justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Why each channel fits, with action steps you can run this
                  week
                </p>
              </div>
              <span className="rounded-full bg-teal-100 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-teal-800">
          ...
          <section className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Model, range, and the rationale behind it
                </p>
              </div>
              <span className="self-start rounded-2xl border border-teal-200 bg-teal-50 px-4 py-2 text-sm font-semibold text-teal-900 md:self-auto">
          ...
          <section className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Each phase compounds on the previous one
              </p>
            </div>
            <div className="grid gap-4 md:grid-cols-3">
          ...
          <section className="space-y-4">
            <div className="flex items-end justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  The six metrics that tell you GTM is working
                </p>
              </div>
              <span className="rounded-full bg-teal-100 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-teal-800">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {result.kpis.map((kpi) => (
                <KpiCard key={`${kpi.metric}-${kpi.target}`} kpi={kpi} />
              ))}
            </div>
          </section>
        </article>
